...
if($templateCatalog == true) {
	$categoryInfo = $qls->shared_SQL->select('*', 'table_template_object_category');
} else {
...
$templates = array();
if($templateCatalog == true) {
	$results = $qls->shared_SQL->select('*', 'table_template_object_templates', array('templateCategory_id' => array('<>', null)), 'templateName ASC');
} else {
	$results = $qls->app_SQL->select('*', 'table_object_templates', array('templateCategory_id' => array('<>', null)), 'templateName ASC');
}
