...
function buildPath($peer, $peerFace, $peerDepth, $object, $objectFace, $objectDepth, &$qls){
	...
	//$children = buildPartitions($peer['parent_id'], $peer['parent_face'], $object, $objectFace, $objectDepth, $qls);
	$value = array(
		3,
		$peer['id'],
		$peerFace,
	...
	$value = implode('-', $value);
	/*
	if($peerTemplateType == 'Insert') {
		if($peerTemplateFunction == 'Passive') {
			$insertNamePrefix = $peer['name'].'.';
		} else {
			$insertNamePrefix = $peer['name'];
		}
	} else {
	...
	*/
	...
	//array_push($children, array(
	//	'value' => $value,
	//	'text' => $insertNamePrefix.$partitionCompatibility['portPrefix'].$portStart.'-'.$portEnd
	//));
	array_unshift($returnArray, array(
		'selected' => $value,
		'children' => $children
...
//function loopDetected(&$qls, $inverseConnectorAttributePrefix, $cable, $elementID, $elementFace, $elementDepth, $elementPortIndex){
	// 'b', cable(17), 34, 0, 1, 0
	//$id = $cable[$inverseConnectorAttributePrefix.'_object_id'];		// 26
	//$face = $cable[$inverseConnectorAttributePrefix.'_object_face'];	// 0
	//$depth = $cable[$inverseConnectorAttributePrefix.'_object_depth'];	// 0
	//$port = $cable[$inverseConnectorAttributePrefix.'_port_id'];		// 1
	...
	while($row = $qls->SQL->fetch_assoc($query)) {
		...
		if($row['partitionType'] == 'Connectable') {
			if($row['portTotal'] == $objectCompatibility['portTotal']) {
				...
				if($addChild) {
					$trunkedFlag = isObjectTrunked($element['id'], $row['side'], $row['depth'], $qls) ? '*' : '';
					if($elementType == 'Insert') {
						if($elementFunction == 'Passive') {
							$insertNamePrefix = $element['name'].'.';
						} else {
				...
				while($insertCompatibility = $qls->SQL->fetch_assoc($queryCompatibility)) {
					...
					if($addChild) {
						$value = array(
							...
							$insert['id'],
							0,
							$insertCompatibility['depth'],
							0
						);
						
						$portNameFormat = json_decode($insertCompatibility['portNameFormat'], true);
						$portTotal = $insertCompatibility['portLayoutX']*$insertCompatibility['portLayoutY'];
						$firstIndex = 0;
						$lastIndex = $portTotal - 1;
						$firstPortName = $qls->App->generatePortName($portNameFormat, $firstIndex, $portTotal);
						$lastPortName = $qls->App->generatePortName($portNameFormat, $lastIndex, $portTotal);
						...
						array_push($treeArray, array(
							'id' => $value,
							'text' => $insert['name'].'.'.$firstPortName.'-'.$lastPortName.$trunkedFlag,
							'parent' => 'O'.$elementID,
							'type' => 'port',
							'data' => array('globalID' => $value)
						));
